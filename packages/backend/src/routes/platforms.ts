      ...
      try {
        const platform = await server.prisma.platform.create({
          data: {
            ...
            releaseDate: platformData.releaseDate ? new Date(platformData.releaseDate) : null
          }
        })
    ...
    handler: async (request: AuthenticatedRequest, reply: FastifyReply) => {
      ...
      try {
        const platform = await server.prisma.platform.update({
          ...
          data: {
            ...platformData,
            releaseDate: platformData.releaseDate ? new Date(platformData.releaseDate) : undefined
          }
        })
